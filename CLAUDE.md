...
- Handlers (`app/routes/`) use `cwfr_add_handlers()` and migrations (`app/migrations/`) use `cwfr_add_migrations()` — both in `core/cmake/app.cmake`, which is also installed with the CMake package. One `.so` is built per `.c` file; handler output stays `exec/handlers/<sub>/lib_<name>.so`, so `config.json` paths are stable.
- The framework is a single shared library, `libcwfr_framework.so` (`core/framework_shared/`), aggregating the whole core and **nothing** from the app — so it can be built and installed once and reused. `cwfr`/`migrate` link it; handler/migration `.so` modules resolve framework symbols from that one shared instance at runtime (shared state: db pools, config, i18n). `CWFR_EXTRA_FW_LIBS` still exists as an extension point but is left empty.
- The app is its own shared library, `libapp.so` (`app/CMakeLists.txt`), whole-archiving `models middlewares handler_context auth mybroadcast` plus `app/app_init.c`. `cwfr` dlopens it from `main.modules` in `config.json` and calls `app_init()` before parsing `servers` — that registers the middlewares and the `httpctx`/`wsctx` `user_data` destructors the core used to resolve at link time.
- `backend/app/` builds two ways from the same files: `add_subdirectory(app)` from `backend/CMakeLists.txt`, or standalone against an installed framework (`cmake -S backend/app -B build -Dcwfr_DIR=<prefix>/lib/cmake/cwfr`). The standalone preamble is one `if(CMAKE_SOURCE_DIR STREQUAL CMAKE_CURRENT_SOURCE_DIR)` block.
- Install rules for everything the framework owns (binaries, `libcwfr_framework.so`, the flat `include/cwfr` headers, the `find_package(cwfr)` package) live in `core/cmake/install.cmake`. See `core/INSTALL.md` §3 and §8.
...
- **Middleware** (`app/middlewares/`): functions returning `int` (0=reject, 1=allow), invoked with the `middleware()` macro; `app/app_init.c` registers them by name for `config.json`.
