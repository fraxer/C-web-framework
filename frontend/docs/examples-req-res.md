...
Готовые примеры обработки HTTP- и WebSocket-запросов: чтение параметров, заголовков,
cookie и тела запроса, отправка ответов, файлов и редиректов. Полное описание функций —
в разделах [HTTP-запросы](/requests), [HTTP-ответы](/responses) и
[Получение данных от клиента](/payload).

## GET

Возвращает строку как тело ответа через `send_data`:
...
`get_payload` возвращает всё тело запроса одной строкой. Подходит для `text/plain` и
сырых данных; результат нужно освободить через `free`:
...
`get_payloadf` извлекает отдельное поле по имени из `multipart/form-data` или
...
`get_payload_file` возвращает загруженный файл как `file_content_t`. Метод
`make_file(dir, name)` сохраняет файл на диск (имя `NULL` — оставить оригинальное),
а `content` читает байты в память. Проверяйте флаг `.ok`:
...
void post(httpctx_t* ctx) {
    ...
    // make_file(dir, name) пишет данные на диск и возвращает открытый file_t
...
То же, что и `get_payload_file`, но файл выбирается по ключу поля из
`multipart/form-data`:
...
`get_payload_json` парсит тело запроса (`application/json`) и возвращает
`json_doc_t*`. Корректность проверяется через `json_ok`:
...
Парсит JSON из отдельного поля `multipart/form-data` по ключу:
...
Параметры строки запроса и параметры маршрута читаются одинаково — функцией
`query_param_char` из `query.h`, по одному и тому же `ctx->request->query_`.
Выходной параметр `ok` отличает «параметра нет» от пустого значения:
...
`get_cookie` возвращает значение cookie по имени или `NULL`:
...
`add_cookie` добавляет заголовок `Set-Cookie` через структуру `cookie_t`:
...
`redirect(url, code)` отправляет перенаправление с указанным кодом:
...
`send_default(code)` отправляет стандартный ответ с указанным кодом состояния без тела:
...
`get_header` возвращает структуру `http_header_t*` или `NULL`:
...
`add_header(key, value)` добавляет заголовок ответа:
...
`send_file(path)` отдаёт файл клиенту:
...
`switch_to_websockets` переключает HTTP-соединение на WebSocket. Дальнейшие сообщения
обрабатываются WebSocket-обработчиками `wsctx_t*`:
...
Текстовое тело сообщения читается функцией `websocketsrequest_payload(protocol)` и
возвращается через `send_text`:
...
Параметры запроса WebSocket доступны через `protocol->query_`. Структуру протокола
нужно привести к `websockets_protocol_resource_t*` — к тому же `query_param_char`
читаются и параметры строки, и параметры маршрута:
