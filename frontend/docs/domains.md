...
- **Регистр не учитывается.** Выражения компилируются с флагом `PCRE_CASELESS`, поэтому `[a-z]` в шаблоне совпадёт и с заглавными буквами, а `(API|WWW).example.com` примет `api.example.com`. То же правило действует и для точных имён — см. [Как происходит сопоставление](#как-происходит-сопоставление).
