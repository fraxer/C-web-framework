...
- Контроль перегрузки NewReno или CUBIC с псевдо-линейным планировщиком отправки (BBR — в планах)
...
|----------|--------------|----------|
| `http3_max_connections` | `65536` | Глобальный лимит QUIC-соединений в процессе (64–4000000; `0` недопустим) |
...
| `http3_cc` | `newreno` | Алгоритм контроля перегрузки: `newreno` или `cubic` |
| `http3_pacing` | `true` | Размазывать отправку по времени вместо залпа |
...
влияет только на новые соединения. Любое значение, кроме `newreno` и `cubic`,
...
| **CUBIC / BBR** | Частично | CUBIC выбирается через `http3_cc`; BBR пока не поддерживается |
...
| **qlog** | Да | Диагностический журнал QUIC |
