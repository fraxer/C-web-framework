...
Этого достаточно: при наличии секции `tls` сервер начинает принимать h2-соединения. Дополнительная конфигурация нужна только для [настройки поведения](#настроика) и для [h2c](#h2c-через-plaintext).
...
| `http2_request_timeout_sec` | `120` | Таймаут ожидания тела запроса и завершения блока заголовков (сек). `0` — отключить |
...
`http2_idle_timeout_sec` — соединение закрывается, когда в нём нет открытых потоков и активности не было N секунд. Соединение с идущим запросом или недописанным ответом «простаивающим» не считается. Перед закрытием сервер отправляет `GOAWAY`.

`http2_request_timeout_sec` — незавершённый запрос сбрасывается с `RST_STREAM(CANCEL)`, если в течение N секунд не поступали данные тела. PING, пустые DATA, padding и трафик других потоков этот срок не продлевают. Блок HEADERS/CONTINUATION должен завершиться за N секунд после первого HEADERS; иначе закрывается соединение, поскольку блок препятствует обработке остальных потоков. Таймаут не распространяется на WebSocket-туннели и запросы, уже переданные обработчику. Для клиентов с долгими паузами при загрузке увеличьте значение или задайте `0`.
...
| **Приоритеты (RFC 9218)** | Не реализованы | Заголовок `priority` и `PRIORITY_UPDATE` здесь игнорируются; готовые потоки планируются по кругу с шагом `http2_write_quantum`. Схема реализована в [HTTP/3](/http3#приоритеты-rfc-9218) |
