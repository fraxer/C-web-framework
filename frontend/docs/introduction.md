...
* HTTP/2 (RFC 9113) поверх TLS через ALPN: мультиплексирование, управление потоком, h2c upgrade, трейлеры, 103 Early Hints, WebSocket-over-h2
* HTTP/3 (RFC 9114) поверх QUIC: транспорт на UDP, 0-RTT-готовность, сосуществование с HTTP/1.1 и HTTP/2 через Alt-Svc
...
* **PCRE2** 10.x (библиотека регулярных выражений)
