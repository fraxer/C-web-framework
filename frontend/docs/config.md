...
Он же — запасной каталог для теневых копий пересобранных обработчиков, когда каталог с самим `.so` недоступен на запись (см. [Горячая перезагрузка](/hot-reload#пересобранныи-обработчик)).
...
Настройки логирования. Поля `enabled` и `level` обязательны, `access` — нет. Журнал идёт в **syslog** — читается через `journalctl -t cwfr`, а не из stdout.
...
Access-лог: одна запись на каждый отданный ответ — кто, что и с каким кодом запросил. Необязательный ключ, по умолчанию `false`.

Это журнал **обращений**, а не событий сервера, и от `enabled` он не зависит: записи о запросах без отладочного шума — обычный случай, а не экзотика. Чтобы `journalctl -t cwfr` не превратился в кашу, access-лог идёт в **отдельный syslog-facility** `local7`:

```bash
journalctl -t cwfr SYSLOG_FACILITY=23 -o cat   # обращения, по строке на запрос
journalctl -t cwfr SYSLOG_FACILITY=1           # события сервера
```

Формат фиксированный — Apache `vhost_combined` плюс длительность обработки в конце:

```
example.com 203.0.113.7 - - [08/Sep/2026:22:41:07 +0700] "GET /page?a=1 HTTP/2" 200 1234 "https://ref/" "Mozilla/5.0" 0.002
```

Поля по порядку: вхост (значение `Host`), адрес клиента, два прочерка на месте `identd` и HTTP-пользователя, время, строка запроса, код ответа, размер тела, `Referer`, `User-Agent`, длительность в секундах. Для GoAccess:
...
Записи одинаковы для HTTP/1.1, HTTP/2 и HTTP/3, и покрывают всё, что отдаёт вхост, — статику, ответы обработчиков, `304`, `429` от rate limit, редиректы. Размер тела — это байты, реально ушедшие на провод: у сжатого ответа он сжатый, у `Range` — размер куска, у `304` — ноль. Строка запроса — та, что прислал клиент: у редиректа записывается исходный путь, а не цель.

Что в записи **не** попадает:

* запрос, который парсер отверг до конца чтения (битая строка запроса, неизвестный `Host`), — метод и путь к этому моменту уже уничтожены, на их месте прочерки;
* ответ, который не удалось дописать в сокет: оборванный ответ не считается отданным.

#### Доставка пачками

Записи копятся в буфере воркера и уходят в syslog **пачкой** — одна отправка на пачку, не на запрос. Это не оптимизация ради оптимизации: journald берёт плату за *запись журнала*, а не за байты, и одна отправка на ответ стоила 4.3 мкс — больше, чем весь остальной ответ. Пачка из шестидесяти записей стоит около 60 нс на запись.

Что из этого следует на практике:

* **Одна запись журнала может содержать несколько строк.** `journalctl -o cat` — то, что скармливают анализатору, — печатает их как отдельные строки, ровно по одной на запрос; обычный вывод `journalctl` показывает продолжения с отступом. Каждая строка несёт своё время, поэтому от того, в какую пачку она попала, не зависит ничего.
* **Запись может отстать примерно на полсекунды.** Буфер сбрасывается, когда заполнится, по таймеру воркера (тик — 500 мс) и при остановке. На тихом сервере это верхняя граница задержки; ни одна запись не теряется, в том числе при перезагрузке конфигурации.
* **Порядок между воркерами — с точностью до пачки.** Внутри одного соединения порядок точный: соединение обслуживает один воркер. То же свойство у пер-воркерных буферов nginx.

::: tip Цена
На синтетическом бенчмарке статики (`wrk`, keep-alive) включённый access-лог стоит около **5 %** пропускной способности: 159.6k → 151.9k запросов в секунду. По профилю на воркер приходится 1.7 %, из них 0.6 % — сама доставка; остальное съедает journald в соседнем процессе.

До введения пачек — по отправке на запрос — то же самое стоило 24 % (159.6k → 121.6k), а на более быстрой машине доходило до половины. Батчинг системных вызовов (`sendmmsg`) здесь не помогает: дорога обработка каждой записи журналом, а не сам вызов.

С `false` цены нет вообще: флаг проверяется до того, как что-либо форматируется, и до чтения часов; на бенчмарке выключенный лог неотличим от сборки без него (158.9k против 159.6k).
:::

### modules <Badge type="info" text="массив строк"/>

Разделяемые библиотеки приложения, которые `cwfr` и `migrate` загружают при старте. Необязательный ключ; порядок элементов — порядок загрузки.

```json
"modules": ["/srv/myapp/lib/cwfr/libapp.so"]
```

Это единственная точка, через которую приложение сообщает ядру то, чего оно не может знать само:

* **имена middleware**, на которые ссылаются `servers.*.http.middlewares` и `servers.*.websockets.middlewares`;
* **деструкторы** для данных, которые приложение вешает на `httpctx_t`/`wsctx_t` через `httpctx_set_user_data()`.

Раньше это были символы, разрешаемые на этапе компоновки, из-за чего код приложения приходилось вкомпилировать в `libcwfr_framework.so`. Теперь фреймворк собирается и устанавливается один раз, ничего не зная о приложении, а обработчики пишутся и пересобираются потом.

#### Контракт модуля

Библиотека обязана экспортировать `int app_init(void)` — возврат `0` прекращает запуск.
...
int app_init(void) {
    /* Один владелец на процесс: с несколькими модулями второй получит 0. */
...
`app_init()` вызывается до того, как разобранная конфигурация опубликована, поэтому логгер на этот момент ещё молчит (`log_message()` выходит, пока `env()` возвращает `NULL`). Пишите об ошибках в `stderr` — иначе сообщение потеряется, и от неудачного старта останется только ненулевой код возврата. Само ядро свои ошибки загрузки дублирует в `stderr` именно по этой причине.
:::

#### Как загружается

Путь передаётся в `dlopen()` **как есть** — ровно так же, как поле `"file"` у маршрута. Абсолютный путь берётся буквально, относительный отсчитывается от текущего каталога процесса, а имя без `/` ищется по обычным правилам динамического загрузчика (`LD_LIBRARY_PATH`, `RPATH`, `ldconfig`). Относительные пути в сервисных юнитах — частый источник «файл не найден» и в норме не нужны.

Флаги — `RTLD_NOW | RTLD_LOCAL`:

* `RTLD_NOW` — все неразрешённые символы модуля диагностируются здесь, с путём в сообщении, а не позже как срыв ленивого связывания в рабочем потоке;
* `RTLD_LOCAL` — символы модуля не попадают в глобальную область поиска, поэтому [несколько модулей](#несколько-модулеи) не склеиваются по одинаковым именам. Обработчикам это не мешает: `.so` обработчика записывает модуль в `DT_NEEDED`, и загрузчик находит уже загруженный экземпляр по `SONAME` независимо от области видимости.

#### Порядок относительно остальной конфигурации

`app_init()` вызывается **после** разбора `main` и **до** разбора `servers`. Иначе быть не может: маршрут ссылается на middleware по имени, а имя появляется в реестре только после регистрации. Неизвестное имя — ошибка конфигурации, и сервер не поднимется.

`migrate` делает то же самое и в том же порядке: он тоже разбирает `servers`, а значит тоже нуждается в заполненном реестре.

#### Несколько модулей

Модулей может быть сколько угодно — они загружаются в порядке массива, и `app_init()` каждого вызывается сразу после его загрузки. Это штатная раскладка для монорепозитория, где приложение собрано из нескольких независимых частей:
...
Что при этом общее на весь процесс, а что принадлежит модулю:

| | Область | Конфликт |
|---|---|---|
| Имена middleware | общий реестр процесса | повторное имя отклоняется, `app_init()` вернёт `0` |
| Деструктор `ctx->user_data` | один на процесс | второй модуль с **другой** функцией получает `0` и сообщение |
| Символы модуля (`.so`) | свои у каждого модуля | нет: модули загружаются с `RTLD_LOCAL` |

Изоляция символов существенна: два модуля вполне могут содержать по своей функции `user_create()`, и без неё второй молча связался бы с реализацией первого. `RTLD_LOCAL` этому мешает и ничего не ломает — обработчик находит свой модуль через `DT_NEEDED`, а не через глобальную область видимости.

::: tip Владелец `ctx->user_data` — один
`httpctx_set_user_data_free()` и `wsctx_set_user_data_free()` возвращают `0`, если деструктор уже занят другой функцией, и печатают об этом. Повторная регистрация **той же** функции успешна — иначе `app_init()` нельзя было бы вызывать заново при перезагрузке.

Проверяйте возврат: без проверки конфликт останется сообщением в журнале, а сервер поднимется с чужим деструктором для половины запросов.
:::

#### Перезагрузка конфигурации

При перезагрузке (`SIGUSR1`) реестр middleware очищается и `app_init()` вызывается заново, поэтому функция **должна быть идемпотентной**: повторная регистрация того же имени возвращает `0` и прекратит запуск, если писать её с накоплением состояния.

Вызывается она при этом **дважды**: сначала на проверочном проходе, который решает, годна ли новая конфигурация, потом на настоящем. Оба раза — с чистым реестром, так что дублей не возникает; но всё, что `app_init()` делает помимо регистрации, произойдёт два раза за перезагрузку.

Если модуль был пересобран, `app_init()` вызывается у **нового** экземпляра, и статические переменные у него свои: состояние между поколениями не переезжает. Всё, что должно пережить перезагрузку, храните вне модуля.

Библиотека принадлежит поколению конфигурации: пересобранная на месте, она загружается заново — копией, под `SONAME` этого поколения, — а прежняя выгружается, когда уходит последний поток прежнего поколения. Раньше этого момента её отображение снимать нельзя: рабочие потоки могут исполнять middleware из неё.

::: tip Пересобранный модуль подхватывается по `SIGUSR1`
Менять путь не нужно. Как это устроено, чего стоит и когда всё-таки не срабатывает — в [Горячей перезагрузке → Модуль приложения](/hot-reload#модуль-приложения).

Добавить в одной перезагрузке **и** новое middleware в `app_init()`, **и** маршрут, который на него ссылается, — можно: проверочный проход исполняет `app_init()` **нового** модуля, поэтому имена новой сборки ему видны.
:::

#### Проверки и ошибки

Значение обязано быть массивом непустых строк; нарушение — ошибка конфигурации. Дальше каждый элемент проходит три проверки, и любая из них прекращает запуск с сообщением в `stderr` и в лог:

| Что не так | Сообщение |
|---|---|
| Файл не открывается | `can't load module <путь>: <причина от dlerror>` |
| Нет `app_init()` | `module <путь> exports no app_init()` |
| `app_init()` вернул `0` | `app_init() failed in <путь>` |

Если модуль не экспортирует `app_init()`, но экспортирует `middlewares_init()` — хук, который ядро больше не вызывает, — к сообщению добавляется подсказка о миграции. Без неё ошибка всплыла бы гораздо позже и в постороннем виде: `failed to find middleware <имя>` при разборе `servers`.

Первые две проверки выполняются и **до** перезагрузки, на этапе валидации новой конфигурации, — но только для модулей, ещё не загруженных в этом процессе, и без вызова `app_init()`. Так опечатка в пути отклоняет перезагрузку, пока старое поколение ещё обслуживает запросы. Отказ самого `app_init()` остаётся фатальным: это ошибка приложения, а не конфигурации, и проявляется она на первом же старте.

#### Без этого ключа

Сервер поднимется, но реестр middleware останется пустым, а `ctx->user_data` — без деструктора. Это рабочая конфигурация для раздачи статики или для маршрутов без прикладных middleware; как только конфигурация сошлётся на middleware по имени, старт прервётся.

См. [Middleware](/middleware), а также `core/INSTALL.md` — разделы «The application module» и «Building an application against an installed framework».
...
Конфигурация HTTP-маршрутов, middleware, заголовков и редиректов. Все пять вложенных ключей необязательны.
...
Список middleware, применяемых ко всем HTTP-маршрутам. Имена берутся из реестра приложения — их регистрирует `app_init()` в модуле, указанном в [`main.modules`](#modules); неизвестное имя — ошибка конфигурации.
...
* `file` — путь до `.so` с обработчиком. Библиотека загружается при старте и переиспользуется всеми маршрутами, которые на неё ссылаются. Пересобранная на месте, она подхватывается по `SIGUSR1` — путь менять не нужно (см. [Горячая перезагрузка](/hot-reload#пересобранныи-обработчик))
...
Query-строка запроса переносится на цель, если у самой цели знака `?` нет: `/old?utm_source=ya` уедет на `/index.html?utm_source=ya`. Цель со своими параметрами (`"/old": "/new?b=2"`) остаётся как есть — они заданы осознанно, и склейка двух наборов была бы сюрпризом. Совпадение по-прежнему ищется по пути без query, поэтому `(.*)` в группу query не захватывает.

#### headers <Badge type="info" text="объект"/>

Заголовки, которые вхост ставит на **каждый** свой ответ. Ключ — имя заголовка, значение — строка; пустое значение — ошибка конфигурации.
...
Это то место, где живут заголовки, задаваемые один раз на сайт. Обработчик до них не дотягивается: статику отдаёт ядро, а middleware работает на маршрутах — статика же отдаётся при промахе по ним. Здесь заголовок попадает на всё: на статику, на ответ обработчика, на `send_default(404)`, на `304`, — и одинаково в HTTP/1.1, HTTP/2 и HTTP/3.

Значение из конфигурации — **умолчание**: обработчик, выставивший свой `Referrer-Policy`, сохраняет своё значение, дубля не возникает. Это то же правило, что у `cache_control` у маршрута.
...
Две ссылки в этом примере ведут за пределы конфигурации, и обе обязаны разрешиться, иначе сервер не поднимется:

* `middleware_http_auth` в `http.middlewares` — имя из реестра, который заполняет `app_init()` модуля `/app/build/exec/libapp.so`, объявленного в [`main.modules`](#modules). Без этой строки в `modules` реестр пуст и старт прервётся с `failed to find middleware middleware_http_auth`.
* `file` у маршрутов и задач — пути к обработчикам, которые разрешаются `dlopen()` при старте; отсутствующий файл или неизвестная `function` тоже прекращают запуск.

Значения `secret` показаны как заглушки. Их место — в файле, указанном через [`env_file`](#env-file), а не в самой конфигурации.
