...
Отдавать готовый `<файл>.gz`, если он лежит рядом с файлом. Сборка фронтенда обычно умеет их писать, и тогда сервер не сжимает вообще ничего: тело едет с диска, с `Content-Length` вместо `chunked`.
