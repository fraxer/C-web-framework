...
```bash
journalctl -t cwfr SYSLOG_FACILITY=23   # обращения
...
::: warning Цена
Каждая запись — синхронная отправка в syslog. На синтетическом бенчмарке статики (`wrk`, keep-alive, один воркер) включённый access-лог стоит примерно половину пропускной способности: 273k → 121k запросов в секунду. Для реального сайта это далеко за пределами нагрузки, но на нагрузочном стенде разницу видно. С `false` цены нет вообще: флаг проверяется до того, как что-либо форматируется, и до чтения часов.
:::
