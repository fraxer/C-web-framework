...
Сама библиотека при этом **не выгружается**. Старые рабочие потоки могут в этот момент исполнять middleware из неё, так что снимать её отображение небезопасно ни при каких условиях.

::: danger Пересобранный модуль требует рестарта
Перезагрузка конфигурации не подхватит пересобранный `.so` — в процессе останется прежний код. То же ограничение действует и для обработчиков; обойти его можно, положив новую сборку под новым именем и указав новый путь — подробности в [Горячей перезагрузке](/hot-reload#обновление-кода-без-рестарта).

Более того, добавить в одной перезагрузке **и** новый модуль, **и** маршрут с его middleware не получится: перед перезагрузкой конфигурация проверяется на корректность со **старым** реестром, новое имя в нём не найдётся, и перезагрузка будет отклонена — работать продолжит прежняя конфигурация. Нужен рестарт.
:::
