...
`http3_pacing` spreads sending instead of handing the window to the network in one piece. The opening flight goes out whole (the burst budget is never below the **initial window**, so a raised `http3_initcwnd_packets` gets the burst it asked for), and beyond that the sender may run at most **one millisecond of its current rate** ahead of its own schedule — the same quantum Linux's `fq` scheduler uses, and never more than the congestion window. This does not change the rate; it changes how the flight is cut up. The budget used to be frozen at the initial window forever, which on a fast path chopped a transfer into pieces of three or four datagrams: extra syscalls and short GSO runs for nothing, with no benefit to any queue. Acknowledgements and PTO probes are never delayed. There is little reason to turn this off outside debugging.
