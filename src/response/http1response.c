...
void http1response_datan(http1response_t* response, const char* data, size_t length) {
    http1response_header_add_content_length(response, length);

    response->header_add_content_type(response, "text/html; charset=UTF-8", 24);
    // response->header_add(response, "Connection", "Keep-Alive");
    response->header_add(response, "Cache-Control", "no-store, no-cache");
...
int http1response_filen(http1response_t* response, const char* path, size_t length) {
    ...
    response->file_.fd = open(path, O_RDONLY);
