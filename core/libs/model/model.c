...
static jsontok_t* __model_json_create_object(void* arg, char** display_fields, jsondoc_t* doc);
...
jsontok_t* model_to_json(void* arg, jsondoc_t* document, ...) {
    ...
    return __model_json_create_object(arg, display_fields, document);
}
...
char* model_stringify(void* arg, ...) {
    ...
    jsontok_t* object = __model_json_create_object(arg, display_fields, doc);
    if (object == NULL) {
        json_free(doc);
        return NULL;
    }
...
jsontok_t* __model_json_create_object(void* arg, char** display_fields, jsondoc_t* doc) {
    if (arg == NULL) return NULL;
