...
void payload_json_post(http1request_t* request, http1response_t* response) {
    ...
    // for (jsonit_t it = json_init_it(object); !json_end_it(&it); it = json_next_it(&it)) {
    //     if (strcmp(json_it_key(&it), "mykey") == 0) {
    //         json_it_erase(&it);
    //     }
    // }

    jsontok_t* array = json_object_get(object, "array");

    // jsonit_t it = json_init_it(array);
    // while (!json_end_it(&it)) {
    //     printf("%p\n", (void*)json_it_value(&it));
    //     it = json_next_it(&it);
    // }

    for (jsonit_t it = json_init_it(array); !json_end_it(&it); json_next_it(&it)) {
        const int* index = json_it_key(&it);

        // jsontok_t* t = json_it_value(&it);
        // if (json_is_string(t)) {
        //     if (strcmp(json_string(t), "string") == 0) {
        //         json_it_erase(&it);
        //     }
        // }
